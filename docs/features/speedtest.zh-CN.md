...
在「应用设置 → 节点自动处理」（原「节点去重」）中开启 **基于出口IP去重**（`landingIPDedupEnabled`）后，测速获取到各节点的出口 IP 时：
...
在「应用设置 → 节点自动处理」中开启 **连续失败自动删除**（`autoDeleteEnabled`）后，每轮节点检测完成后会自动维护计数：

- 延迟检测**成功清零**、**失败 +1**（超时/错误均计失败；仅测速度不改变计数）。
- 连续失败次数达到阈值（`autoDeleteThreshold`，默认 3 次，范围 1-20）的节点被**自动从节点库删除**（连同订阅关联关系一并清理）。
- 作用范围由 `autoDeleteGroups` 控制：支持**分组多选**，留空表示作用于全部分组；不在所选分组内的节点不参与计数。
- 删除操作异步执行并写入日志（含被删节点名与作用分组），不可恢复——请谨慎设置阈值。
